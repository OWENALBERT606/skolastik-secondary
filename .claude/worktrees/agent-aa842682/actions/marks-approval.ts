...
export async function getMarksApprovalsData(schoolId: string) {
  try {
    ...
    for (const ss of streamSubjects) {
      ...
      const totalMarks = examDraft + examSubmitted + examApproved + aoiDraft + aoiSubmitted + aoiApproved;
      const allApproved = totalMarks > 0 && examDraft === 0 && examSubmitted === 0 && aoiDraft === 0 && aoiSubmitted === 0;
      ...
      streamEntry.subjects.push({
        ...
        aoiDraft,  aoiSubmitted,  aoiApproved,
    ...
    return {
      ...
      data: {
        ...
        students: ss.studentEnrollments.map((se) => {
          ...
          const hasSubmitted = allStatuses.some(s => s === MarkStatus.SUBMITTED);
  ...
  try {
    ...
    const [e, a, u] = await Promise.all([
      db.examMark.updateMany({
        where: { studentSubjectEnrollment: { streamSubjectId }, status: MarkStatus.SUBMITTED },
...
export async function approveStreamMarks(data: {
  ...
  try {
    ...
    const [e, a, u] = await Promise.all([
      db.examMark.updateMany({
        where: { studentSubjectEnrollment: { streamSubjectId: { in: ssIds } }, status: MarkStatus.SUBMITTED },
        data:  { status: MarkStatus.APPROVED, approvedById: approverId, approvedAt: now },
      }),
      db.aOIScore.updateMany({
        where: { studentSubjectEnrollment: { streamSubjectId: { in: ssIds } }, status: MarkStatus.SUBMITTED },
