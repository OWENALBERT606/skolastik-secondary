// components/academic-terms/CreateAcademicTermDialog.tsx
"use client";
...
import { getAcademicYearsBySchool } from "@/actions/years";
import { createAcademicTerm } from "@/actions/terms";
...
interface CreateAcademicTermDialogProps {
  open: boolean;
...
export default function CreateAcademicTermDialog({
  open,
  onOpenChange,
  schoolId,
  onSuccess,
}: CreateAcademicTermDialogProps) {
  ...
  // Load academic years when dialog opens
  useEffect(() => {
    if (open) {
      loadAcademicYears();
    }
  ...
  const loadAcademicYears = async () => {
    ...
    try {
      const result = await getAcademicYearsBySchool(schoolId);
      if (result.ok) {
      ...
      <DialogContent className="sm:max-w-[600px] bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
        <DialogHeader>
          ...
          <DialogDescription className="text-gray-600 dark:text-gray-400">
            Add a new academic term to an academic year. Each academic year can have up to 3 terms.
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-gray-900 dark:text-gray-100">Academic Year *</FormLabel>
                  <Select
                  ...
                  <FormDescription className="text-gray-600 dark:text-gray-400">
                    Select the academic year for this term
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
