...
type SubjectRow = {
  ...
  aoiScores:       (number | null)[];   // individual AOI scores out of 3, up to 3
  summativeContribution: number | null; // 80% portion
...
export type ReportCardData = {
  ...
  isPublished:         boolean;
  subjects:            SubjectRow[];
  examWeights:         { bot: number; mte: number; eot: number; aoi: number; summative: number };
};
...
async function fetchStreamReportCards(streamId: string, termId: string): Promise<ReportCardData[]> {
  const res = await fetch(`/api/report-cards?streamId=${streamId}&termId=${termId}`);
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                ...
                {(() => {
                  ...
                  const examCols = [hasBOT, hasMTE, hasEOT].filter(Boolean).length;
                  return (
...
export default function ReportCardPreview({
  streamId, termId, format, theme, school, academicYear, termName, onClose,
}: Props) {
  ...
  useEffect(() => {
    setLoading(true);
    fetchStreamReportCards(streamId, termId)
      .then(setCards)
