...
function SubjectCard({
  ...
  return (
    <div className={`rounded-xl border hover:shadow-md transition-all group ${
      ...
      {/* Enroll-to-next-term: only for active ACTIVE-assignment subjects */}
      {canManage && (
...
export default function TeacherDashboardClient({ teacher, subjects, summary, slug, communications, isDOS, userId }: Props) {
  const [activeTab, setActiveTab] = useState<"subjects" | "communications">("subjects");
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 space-y-4">
      ...
      <div className="flex gap-1 border-b border-slate-200 dark:border-slate-700">
        {([
          { key: "subjects"       as const, label: "My Subjects",    icon: BookOpen },
          ...
          >
            ...
            {"badge" in t && t.badge != null && (
              <span className="bg-red-500 text-white text-xs rounded-full px-1.5 py-0.5 font-semibold">{t.badge}</span>
            )}
          </button>
        ))}
      </div>
