...
import {
  ...
  GraduationCap, Layers, Clock, UserX, BarChart3, BookMarked, Search,
} from "lucide-react";
...
function TeacherWorkloadDialog({ open, teacher, slug, onClose }: {
        ...
        <div className="rounded-xl border border-amber-200 bg-amber-50 dark:bg-amber-950/20 dark:border-amber-800 px-5 py-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
            {unassignedSubjects.slice(0, 9).map(ss => (
              <Link key={ss.id}
                href={`/school/${slug}/dos/subjects/${ss.id}`}
                className="flex items-center justify-between bg-white dark:bg-slate-900 rounded-lg px-3 py-2 border border-amber-100 dark:border-amber-900 hover:border-amber-300 dark:hover:border-amber-700 transition-colors group">
                ...
                <ChevronRight className="h-4 w-4 text-slate-300 group-hover:text-amber-500 shrink-0 ml-2" />
              </Link>
            ))}
          </div>
