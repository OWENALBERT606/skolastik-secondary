...
import {
  Bell, Search, ChevronDown, BookMarked, GraduationCap, Check,
  ChevronsUpDown, User, Settings, KeyRound, LogOut,
} from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
...
interface Props {
  session: { name: string; email: string; image?: string | null; hasTeacherRole: boolean };
  slug:    string;
  caps:    Capabilities;
}

export default function DOSNav({ session, slug, caps }: Props) {
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="hidden sm:flex items-center gap-2 hover:bg-zinc-100 dark:hover:bg-slate-800/60 rounded-lg px-2 py-1 transition-colors">
                ...
                <ChevronsUpDown className="h-3.5 w-3.5 text-zinc-400 dark:text-zinc-500 shrink-0" />
              </button>
            </DropdownMenuTrigger>
            ...
            >
              ...
              <DropdownMenuItem asChild className="gap-2 px-3 py-2 text-sm cursor-pointer text-zinc-700 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-white focus:bg-zinc-50 dark:focus:bg-slate-800/60">
                <Link href={`/school/${slug}/dos/profile`}>
                  <User className="w-4 h-4 text-zinc-400 dark:text-zinc-500" />
                  My Profile
                </Link>
              </DropdownMenuItem>
              <DropdownMenuItem asChild className="gap-2 px-3 py-2 text-sm cursor-pointer text-zinc-700 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-white focus:bg-zinc-50 dark:focus:bg-slate-800/60">
                <Link href={`/school/${slug}/dos/settings`}>
