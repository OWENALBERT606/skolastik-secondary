...
import { ChevronRight, ChevronDown, ExternalLink, LogOut, ChevronsUpDown, Plus } from "lucide-react";
...
export default function Sidebar({ user, notifications = [] }: SidebarProps) {
  ...
  const hasPermission = (permission: string): boolean => {
    ...
    if (roles.some((r: string) => r.includes("admin") || r.includes("super"))) return true;
      ...
      <div className="flex h-full max-h-screen flex-col gap-2">
        <div className="flex flex-shrink-0 h-14 items-center border-b px-4 lg:h-[60px] lg:px-6">
          <Link href="/dashboard" className="flex items-center gap-2 font-semibold text-slate-900 dark:text-white">
            <span className="text-primary font-bold text-lg">Skolastik</span>
          </Link>
        </div>
