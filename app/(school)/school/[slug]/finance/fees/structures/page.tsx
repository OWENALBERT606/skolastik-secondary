// app/school/[slug]/finance/fees/structures/page.tsx
import { redirect } from "next/navigation";
import { db }       from "@/prisma/db";
import { getAuthenticatedUser } from "@/config/useAuth";
import FeeStructuresClient from "./components/feestructureClient";
...
export default async function FeeStructuresPage({
  ...
  // All academic years + terms
  const allYears = await db.academicYear.findMany({
    where:   { schoolId },
    include: { terms: { orderBy: { termNumber: "asc" } } },
    orderBy: { year: "desc" },
  });
  if (!allYears.length) redirect(`/school/${slug}/finance/fees`);
  ...
  // Fee structures for selected year (all terms) so client can filter
