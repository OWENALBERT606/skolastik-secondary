...
import EnterProjectScoresDialog from "./enter-project-scores-dialog";
...
export default function SubjectMarksTab({
  ...
  const [showProjectScores,   setShowProjectScores]   = useState(false);
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4">
          ...
          <div className="flex flex-wrap gap-3">
            {[
              ...
              { label: "PROJECT", value: assessmentConfig.hasProject ? assessmentConfig.projectWeight : 0 },
            ]
              .filter((w) => w.value > 0)
              .map(({ label, value }) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        {/* Project Assessment card */}
        {assessmentConfig?.hasProject && (
          <ActionCard
            icon={<FolderOpen className="w-5 h-5 text-purple-600 dark:text-purple-400" />}
            iconBg="bg-purple-100 dark:bg-purple-900/30"
            title="Project Assessment"
            subtitle={`Out of ${assessmentConfig.projectMaxScore} · ${assessmentConfig.projectWeight}% of final mark`}
            progress={stats.totalStudents > 0 ? (stats.withProjectScores / stats.totalStudents) * 100 : 0}
            progressColor="bg-purple-500"
            onClick={() => !marksLocked && setShowProjectScores(true)}
            disabled={marksLocked}
            buttonLabel="Enter Project Scores"
          />
        )}
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                  <tr>
                    ...
                    {assessmentConfig?.hasProject && (
                      <th className="text-right px-4 py-3 font-medium text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wide">Project</th>
                    )}
                  ...
                  {studentSummaries.map((s: any, i: number) => (
                    <tr key={s.enrollmentId} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                      ...
                      {assessmentConfig?.hasProject && (
                        <td className="px-4 py-3 text-right">
                          {s.projectScore != null
                            ? <span className="font-medium text-purple-700 dark:text-purple-400">{s.projectScore}/{assessmentConfig.projectMaxScore}</span>
                            : <span className="text-slate-300 dark:text-slate-600 text-xs">—</span>}
                        </td>
                      )}
      ...
      {showProjectScores && assessmentConfig?.hasProject && (
        <EnterProjectScoresDialog
          open={showProjectScores}
          onOpenChange={(open) => {
            setShowProjectScores(open);
            if (!open) onRefresh();
          }}
          streamSubjectId={streamSubject.id}
          enrollments={enrollments}
          projectMaxScore={assessmentConfig.projectMaxScore ?? 10}
          projectWeight={assessmentConfig.projectWeight ?? 10}
          schoolId={schoolId}
          isLocked={marksLocked ?? false}
        />
      )}
