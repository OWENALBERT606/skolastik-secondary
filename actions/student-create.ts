...
import { ensureGPStreamSubjects } from "./students";
import {
  UserType,
  EnrollmentStatus,
  EnrollmentType,
...
type CreateStudentSmartPayload = {
  ...
  // A-level: specific subject IDs selected by the student
...
export async function createStudentSmart(data: CreateStudentSmartPayload) {
  try {
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      if (data.streamId) {
        // A-level: enroll only in selected subjects' papers, but ALWAYS include
        //   General Paper (isGeneralPaper = true) — compulsory for A-level.
        // O-level: enroll in all COMPULSORY subjects' papers.
        const hasSelectedSubjects = data.selectedSubjectIds && data.selectedSubjectIds.length > 0;

        // Auto-create GP StreamSubject if GP isn't already in the stream
        if (hasSelectedSubjects) {
          await ensureGPStreamSubjects(
            tx, data.streamId!, data.classYearId, termId!, data.schoolId,
          );
        }
        ...
        const streamSubjects = await tx.streamSubject.findMany({
          where: {
            ...
            ...(hasSelectedSubjects
              ? {
                  OR: [
                    // explicitly selected by the form
                    { subjectId: { in: data.selectedSubjectIds } },
                    // always include subjects flagged as General Paper
                    { subject: { isGeneralPaper: true } },
                    // fallback: include by name if no subject is flagged yet
          ...
          include: {
            subject:      { select: { id: true, name: true, code: true, isGeneralPaper: true } },
        ...
        if (streamSubjects.length > 0) {
          await tx.studentSubjectEnrollment.createMany({
            data: streamSubjects.map((ss) => ({
              ...
              isCompulsory:    ss.subjectType === "COMPULSORY" || ss.subject.isGeneralPaper,
              isAutoEnrolled:  true,
