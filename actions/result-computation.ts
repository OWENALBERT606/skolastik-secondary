...
function getALevelMajorGrade(pct: number): GradeResult {
  // UACE principal-pass grading using UCE grade notation
  // D1=A=6pts, D2=B=5pts, C3=C=4pts, C4=D=3pts, C5=E=2pts, C6=O=1pt, P7-F9=F=0pts
  if (pct >= 80) return { grade: "D1", letter: "A", descriptor: "Very Good",    points: 6 };
  if (pct >= 75) return { grade: "D2", letter: "B", descriptor: "Good",         points: 5 };
  if (pct >= 70) return { grade: "C3", letter: "C", descriptor: "Credit",       points: 4 };
  if (pct >= 65) return { grade: "C4", letter: "D", descriptor: "Pass",         points: 3 };
  if (pct >= 60) return { grade: "C5", letter: "E", descriptor: "Pass",         points: 2 };
  if (pct >= 55) return { grade: "C6", letter: "O", descriptor: "Ordinary",     points: 1 };
  if (pct >= 50) return { grade: "P7", letter: "F", descriptor: "Fail",         points: 0 };
  if (pct >= 40) return { grade: "P8", letter: "F", descriptor: "Fail",         points: 0 };
  return               { grade: "F9", letter: "F", descriptor: "Fail",          points: 0 };
}
...
export async function computeStreamSubjectResults(data: {
  ...
  try {
    ...
    for (const se of ss.studentEnrollments) {
      ...
      const aoiConfigCount  = cfg.maxAOICount;
      const aoiRawAverage   = aoiConfigCount > 0 ? aoiTotal / aoiConfigCount : 0;
      // AOI counts only if the config has AOI enabled AND at least 1 approved score exists
      const hasAoiScores    = cfg.hasAOI && aoiTopicScores.length > 0;

      // ── Guard: AOI required ───────────────────────────────────────────────
      // If the class config has AOI enabled, every student must have at least
      // 1 approved AOI score before results can be computed. Without it the
      // 20% AOI weight cannot be fairly applied.
      if (cfg.hasAOI && !hasAoiScores) {
        // Skip this student — caller will see a reduced computed count and
        // can surface the warning to the user.
        continue;
      }
      const aoiContribution = hasAoiScores
      ...
      // Build list of exams that actually have marks submitted
      ...
      const existingResult = await db.subjectResult.findUnique({
        where:  { studentSubjectEnrollmentId: se.id },
        select: { projectScore: true, projectOutOf: true },
      });
      const projectScore = existingResult?.projectScore ?? null;
      const projectOutOf = existingResult?.projectOutOf ?? cfgAny.projectMaxScore ?? 10;
      const projectContribution = cfgAny.hasProject && projectScore !== null && projectOutOf > 0
        ? (projectScore / projectOutOf) * (cfgAny.projectWeight ?? 0)
        : 0;

      // ── Dynamic weight redistribution ────────────────────────────────────
      // If AOI was not done (no scores), the full 100% goes to summative exams.
      // If AOI was done, AOI takes its configured weight and summative fills the rest.
      // Within summative, only exams that actually have marks are included;
      // their configured weights are normalised to sum to the summative pool.
      const effectiveAoiWeight      = hasAoiScores ? cfg.aoiWeight : 0;
      const effectiveProjectWeight  = cfgAny.hasProject && projectScore !== null
        ? (cfgAny.projectWeight ?? 0) : 0;
      const summativePool           = 100 - effectiveAoiWeight - effectiveProjectWeight;

      let summativeContribution = 0;
      if (activeExams.length > 0) {
        const rawWeightSum = activeExams.reduce((s, e) => s + e.weight, 0);
        // Normalise: each exam's share of the summative pool is proportional to its
        // configured weight relative to the total configured weight of present exams.
        const weightedAvg = rawWeightSum > 0
          ? activeExams.reduce((s, e) => s + (e.pct * e.weight), 0) / rawWeightSum
          : 0;
        summativeContribution = (weightedAvg / 100) * summativePool;
      }

      const totalPercentage = Math.min(100, aoiContribution + summativeContribution + projectContribution);
      ...
      await db.subjectResult.upsert({
        ...
        update: {
          ...
          projectOutOf:        (cfg as any).hasProject ? projectOutOf : null,
          projectContribution: projectContribution > 0 ? projectContribution : null,
    ...
    const total = ss.studentEnrollments.length;
    const skipped = total - computed;
    return {
      ok:      true as const,
      message: skipped > 0
        ? `Computed results for ${computed} student(s). ${skipped} skipped — AOI scores not yet registered.`
        : `Computed results for ${computed} student(s)`,
      count:   computed,
    };
  } catch (error: any) {
...
// ════════════════════════════════════════════════════════════════════════════
// COMPUTE RESULTS + GENERATE REPORT CARD FOR A SINGLE STUDENT
// Used from the student detail page to generate / regenerate one student's card.
// Recomputes the whole stream so positions are correct, then generates the card.
// ════════════════════════════════════════════════════════════════════════════
...
export async function computeAndGenerateStudentReportCard(data: {
  ...
  try {
    ...
    if (!enrollment)          return { ok: false as const, message: "Enrollment not found." };
    if (!enrollment.streamId) return { ok: false as const, message: "This enrollment has no stream — report cards are not applicable." };

    // Recompute subject results for the whole stream (needed for SubjectFinalMark records + positions)
    ...
    // Generate (upsert) report card from the computed results
